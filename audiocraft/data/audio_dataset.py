...
class AudioDataset:
    ...
    def __init__(self,
                 ...
                 audio_encoder_sr: tp.Optional[int] = None,
                 # 视频帧率
                 video_fps: int = 2,
                 video_overlap: int = 2,
        ...
        self.audio_encoder_sr = audio_encoder_sr
    ...
    def __getitem__(self, index: int) -> tp.Union[torch.Tensor, tp.Tuple[torch.Tensor, SegmentInfo]]:
        if self.segment_duration is None:
            ...
            for retry in range(self.max_read_retry):
                ...
                try:
                    ...
                    style_target_frames = int(self.segment_duration * self.audio_encoder_sr)
                    if style_audio.shape[-1] > style_target_frames:
                        style_audio = style_audio[..., :style_target_frames]
                    if self.pad and style_audio.shape[-1] < style_target_frames:
                        style_audio = F.pad(style_audio, (0, style_target_frames - style_audio.shape[-1]))
        ...
        if self.if_add_gobal:
            if self.return_info:
                ...
                return out, style_audio, [local_video, global_video], segment_info
            else:
                return out, style_audio, [local_video, global_video]
        else:
            if self.return_info:
                # Returns the wav and additional information on the wave segment
                return out, style_audio, [video], segment_info
            else:
                return out, style_audio, [video]
    ...
    def collater(self, samples):
        ...
        if self.return_info:
            if len(samples) > 0:
                assert len(samples[0]) == 4  # <--- 修改
                assert isinstance(samples[0][0], torch.Tensor)
                assert isinstance(samples[0][1], torch.Tensor) # style_audio  # <--- 新增断言
                assert isinstance(samples[0][2], list)         # video_list
                assert isinstance(samples[0][3], SegmentInfo)  # info


            wavs = [wav for wav, _, _, _ in samples]
            style_audios = [style_audio for _, style_audio, _, _ in samples] # <--- 新增
            video_lists = [video_list for _, _, video_list, _ in samples] # <--- 修改解包索引
            segment_infos = [copy.deepcopy(info) for _, _, _, info in samples] # <--- 修改解包索引
            wav = torch.stack(wavs)
            style_wav = torch.stack(style_audios) # <--- 新增
            ...
            if len(video_lists[0])==1:
                ...
                return wav, style_wav, [video], segment_infos # <--- 修改
            ...
            # 处理同时包含局部和全局视频的情况
                ...
                return wav, style_wav, [local_video, global_video], segment_infos # <--- 修改
