...
class MusicDataset(InfoAudioDataset):
    ...
    def __getitem__(self, index):
        # 1. 调用父类 (AudioDataset) 的 __getitem__ 方法
        #    父类负责完成所有繁重的音视频同步加载和预处理工作。
        #    执行完毕后，wav 是音频张量, video_list 是包含视频张量的列表, info 是基本信息。
        wav, style_audio, video_list, info = super().__getitem__(index) # <--- 修改
        # 断言确保父类返回了预期的列表格式
        assert isinstance(video_list, list)
        
        
        # 2. == 处理视频情况 1: 只有局部视频 ==
        #    这种情况对应 AudioDataset 中 if_add_gobal=False 的配置。
        if len(video_list)==1:
            video=video_list[0]
             # --- 以下是本类的核心功能：加载和处理音乐元数据 ---
            ...
            # 3. 返回最终的数据样本
            #    注意：video 张量在这里被原封不动地传回，只是重新包装在列表里。
            return wav, style_audio, [video], music_info # <--- 修改
            ...
            # 5. 返回最终的数据样本
            #    注意：local_video 和 global_video 两个张量在这里被原封不动地传回，
            #    并保持了它们在列表中的原始顺序。
            return wav, style_audio, [local_video, global_video], music_info # <--- 修改
