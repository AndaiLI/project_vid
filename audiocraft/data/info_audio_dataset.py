...
class InfoAudioDataset(AudioDataset):
    ...
    def __getitem__(self, index: int) -> tp.Union[torch.Tensor, tp.Tuple[torch.Tensor, SegmentWithAttributes]]:
        if not self.return_info:
            ...
            wav = results[0] # 第一个元素总是内容音频
            assert isinstance(wav, torch.Tensor)
            return wav
        # 1. 调用父类 (AudioDataset) 的 __getitem__ 方法。
        #    这一步执行了所有实际的音视频同步加载工作。
        #    返回的 wav 是音频, video_list 是视频列表, meta 是一个 SegmentInfo 对象。
        wav, style_audio, video_list, meta = super().__getitem__(index) # <--- 修改
        assert isinstance(video_list, list)
        # 2. 核心操作：将父类返回的 meta 对象 (类型为 SegmentInfo)
        #    转换成一个新的 AudioInfo 对象。AudioInfo 继承了 SegmentWithAttributes，
        #    从而满足了格式要求。
        return wav, style_audio, video_list, AudioInfo(**meta.to_dict()) # <--- 修改
