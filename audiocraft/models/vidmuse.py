...
class VidMuse:
    ...
    def generate(self, descriptions_list: tp.List, progress: bool = False, return_tokens: bool = False) \
        ...
        # 1. == 对输入的 video_list 进行断言和解包 ==
        #    这里的参数名 descriptions_list 实际上应该理解为 video_list 或 conditions_list 更为贴切。
        #    它期望接收一个包含两个元素的列表。
        ...
        # 2. == 将列表中的元素分别赋值给局部和全局视频变量 ==
        #    这里的命名 (local_descriptions) 可能是从 AudioCraft 继承而来，
        #    但在 VidMuse 的上下文中，它们代表的是预处理过的视频张量。
    ...
    def _generate_tokens(self, attributes: tp.List,
        ...
        if self.duration <= self.max_duration:
            ...
            while current_gen_offset + prompt_length < total_gen_len:
                ...
                # == 核心：更新 prompt 和视频窗口，为下一次循环做准备 ==
                # 使用当前生成的尾部作为下一次生成的 prompt
                prompt_tokens = gen_tokens[:, :, stride_tokens:]
                prompt_length = prompt_tokens.shape[-1]
                
                # "滑动" 局部视频的窗口，丢弃已经处理过的帧
