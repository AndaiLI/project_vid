...
from transformers import AutoProcessor, CLIPVisionModelWithProjection, VideoMAEModel, AutoFeatureExtractor, AutoModel, ClapModel
...
class LMModel(StreamingModule):
    ...
    def __init__(self, pattern_provider: CodebooksPatternProvider, condition_provider: ConditioningProvider, 
        ...
        # 3. == 创建用于处理 "局部" 视频时序信息的 Transformer ==
    ...
    def compute_audio_emb(self, reference_audio: torch.Tensor) -> torch.Tensor:
        ...
        # --- 获取MERT期望的采样率，用于传递给processor ---
        target_sr = self.audio_processor.sampling_rate
        
        # --- 预处理音频 (不再包含重采样) ---
        if reference_audio.shape[1] > 1:
            reference_audio = reference_audio.mean(dim=1)
        else:
            reference_audio = reference_audio.squeeze(1)
            
        audio_waveforms = [wav.cpu().numpy() for wav in reference_audio]

        max_length_seconds = 10
        max_length_samples = target_sr * max_length_seconds

        # --- 使用音频预处理器 ---
        inputs = self.audio_processor(audio_waveforms, 
        ...
        processed_wav = inputs['input_values'].to(device)
    ...
    def forward(self, sequence: torch.Tensor,
                ...
                reference_audio: tp.Optional[torch.Tensor] = None,
    ...
    def compute_predictions(
            ...
            conditions: tp.List[ConditioningAttributes],
            condition_tensors_list: tp.List,
            reference_audio: torch.Tensor,) -> LMOutput:
        ...
        logits = model(sequence_codes, conditions, condition_tensors_list, reference_audio)  # [B, K, S, card]
    ...
    def generate(self,
        ...
        assert reference_audio is not None, "Reference audio must be provided."
